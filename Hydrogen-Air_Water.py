...
v3 = (v3_air*kg_air + v3_H2*kg_H2)/(kg_air+kg_H2)

water = pm.get('ig.H2O')
p3_water = water.p(T=t3, v=v3)
cv3_water = water.cv(T=t3, v=v3)
cp3_water = water.cp(T=t3, v=v3)
s3_water = water.s(T=t3, v=v3)
gamma3_water = cp3_water/cv3_water
...
cv_avg = ((cv_air_1+cv_air_2+cv_air_3+cv_air_4)*kg_air + (cv_H2_1+cv_H2_2+cv_H2_3+cv_H2_4)*kg_H2 )/(4*(kg_H2+kg_air))
...
p4_water = p3_water*r**(-gamma3_water)
t4_water = t3*r**(1-gamma3_water)
cv4_water = water.cv(T=t4, p=p4_water)
s4_water = water.s(T=t4_water, p=p4_water)
v4_water = water.v(T=t4_water, p=p4_water)
cv_avg_water = ((cv_air_1+cv_air_2)*kg_air + (cv_H2_1+cv_H2_2)*kg_H2 + (cv_H2_3+cv_H2_4)*(kg_air+kg_H2))/(4*(kg_H2+kg_air))
#gamma_avg_34 = (gamma_3+gamma_4)/2
# %%
W = cv_avg*( (t3-t2)-(t4-t1) )
W_water = cv_avg*( (t3-t2)-(t4_water-t1) )
P_spec_water = W_water*rpm/60
P_water = P_spec_water*(kg_air + kg_H2)
